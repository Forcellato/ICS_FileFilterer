...
def main():
    ...
    logger.global_logger.log_ok('Settings parsed successfully.')
    ...
    logger.global_logger.log_ok('Calendars loaded.')
    ...
    logger.global_logger.log_info(f'Total events: {len(events)}.')
    # the opened input files are now useless, this closes all the opened files
    for file in ics_file_list:
        file.close()
        logger.global_logger.log_info(f'File {file.name} is now closed.')
    logger.global_logger.log_ok('ICS input files closed.')
    ...
    th1.join()
    th2.join()
    logger.global_logger.log_ok(f'Exec time: {time() - t}')
